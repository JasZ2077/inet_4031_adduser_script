...
def main():
# attempting to use tty to get around the standard input
    tty = open ("/dev/tty")
    print("Dry Run (Y/N)?")
    mode = tty.readline().strip().upper()
# Dry run code: added some conditions in everywhere it needs os.system(cmd), you need to explicitly type "Y" to dry run.
